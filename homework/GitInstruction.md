...
_**Работа с удаленными репозиториями возможна с помощью команд git clone, git fetch, git push, git pull, git remote**_ 

Для того чтобы отправить измененные файлы в удаленный репозиторий применятся команда git push
